...
def build_progress(experiments: dict[str, Any], datasets: dict[str, Any], operations: dict[str, Any]) -> dict[str, Any]:
    ...
    promotion_actions = decision.get("promotion_actions", []) if isinstance(decision.get("promotion_actions"), list) else []
    top_action = promotion_actions[0].get("label") if promotion_actions and isinstance(promotion_actions[0], dict) else None
    ...
    next_action = top_action or decision.get("next_action") or "Inspect validation health and run seed-repeat LOO before promotion."
    return {
        "summary": {
            "status": status,
            "label": "AutoResearch running" if status == "running" else "Ready for review",
            "percent": percent,
            "foundation_readiness": round(100 * sum(1 for m in milestones if m["state"] == "done") / max(len(milestones), 1)),
            "current_step": f"Best F1 {best_f1 if best_f1 is not None else 'n/a'}; latest F1 {latest_f1 if latest_f1 is not None else 'n/a'}.",
