...
The standalone dashboard is a Vesuvius-native view of configs, experiment runs, prepared-data metadata, fold maps, logs, promotion commands, safe controls, and optional agent chat. It keeps scorecards and the decision brief visible while collapsing long review surfaces by default and containing the experiment ledger in a filterable scroll window.
...
The main page is optimized for long-running research histories:

- The Research Usefulness Leaderboard starts collapsed and provides search, quality verdict, promotion status, and row-limit controls before rendering the ranked candidate table.
...
Interactive run controls are available only when `VESUVIUS_DASHBOARD_ENABLE_RUNS=1` and the server is launched with token auth, unless private tokenless POST mode is explicitly enabled. The backend accepts command IDs, not shell strings; it resolves IDs from the snapshot, requires `safe_to_execute_from_dashboard: true`, rejects artifact-writing commands, runs with `shell=False`, and captures stdout/stderr for display. Unsafe commands remain copy-only. Matrix and ledger actions that only filter or select rows are client-side navigation aids; command execution still goes through the same allowlist.
...
Settings mutation endpoints require dashboard token auth unless `VESUVIUS_DASHBOARD_ALLOW_UNAUTHENTICATED_POSTS=1` is set for private tokenless mode.

### Visual Analysis

Decoded-output visual analysis is opt-in and token-gated unless private tokenless mode is enabled. It lets the configured agent inspect the guarded preview images already generated by the dashboard, such as `probability_map.npy` heatmaps and threshold masks, and return advisory feedback about coherent ink structure, blockiness, flooding, speckles, or noise. The decoded-output gallery starts folded so these heavier review artifacts do not dominate the page or load visual previews until requested.
...
Full-tile evidence and leaderboard rows include `quality_next_actions` and `quality_next_action`. `pass` has no action, `review` asks for full-tile quality review, and `fail` asks for quality remediation before promotion. The standalone UI renders this as a collapsed, searchable Research Usefulness Leaderboard so long ranked lists are reviewable without pushing the run ledger down the page.

`experiments.validation_matrix` is rendered as an action matrix: each populated cell can filter the ledger by train/validation segment or select the best run for that cell. The matrix also reuses safe command inventory entries for validation dry-runs and prepared-data checks.
